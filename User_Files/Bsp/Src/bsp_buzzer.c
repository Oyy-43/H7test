...
static void Buzzer_Apply_Register(void);
...
static void Buzzer_Apply_Register(void)
{
    ...
    if (SYS_Buzzer.TIM_Handler == NULL)
    {
        return;
    }

    if (SYS_Buzzer.Frequency > 0.0f)
    {
        arr = 1000000.0f / SYS_Buzzer.Frequency;
    }

    __HAL_TIM_SetAutoreload(SYS_Buzzer.TIM_Handler, arr);

    float ccr = arr * SYS_Buzzer.Loudness * 0.5f;
    __HAL_TIM_SetCompare(SYS_Buzzer.TIM_Handler, SYS_Buzzer.TIM_Channel, ccr);
}
...
{
    SYS_Buzzer.TIM_Handler = &htim12;
    SYS_Buzzer.TIM_Channel = TIM_CHANNEL_2;
    SYS_Buzzer.Frequency = Basic_Math_Constrain(__Frequency, 0.0f, 20000.0f);
    SYS_Buzzer.Loudness = Basic_Math_Constrain(__Loudness, 0.0f, 1.0f);

    Buzzer_Apply_Register();
    HAL_TIM_PWM_Start(SYS_Buzzer.TIM_Handler, SYS_Buzzer.TIM_Channel);
}
